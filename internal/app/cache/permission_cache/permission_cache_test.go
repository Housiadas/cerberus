...
func newTestStore(t *testing.T) (*Store, *permission.MockStorer, *redis.MockClient) {
	t.Helper()

	mockStorer := permission.NewMockStorer(t)
...
func TestQueryByID_CacheMiss(t *testing.T) {
	...
	id := uuid.New()
	expected := permission.Permission{ID: id}
...
func TestCreate_DelegatesToStorer(t *testing.T) {
	...
	p := permission.Permission{ID: uuid.New()}
