...
func (s *Store) Create(ctx context.Context, acct account.Account) error {
	err := pgsql.NamedExecContext(ctx, s.log, s.db, accountCreateSQL, toAccountDB(acct))
	if err != nil {
		return fmt.Errorf("error account create in db: %w", err)
	}
...
func (s *Store) Update(ctx context.Context, acct account.Account) error {
	err := pgsql.NamedExecContext(ctx, s.log, s.db, accountUpdateSQL, toAccountDB(acct))
	if err != nil {
		return fmt.Errorf("error account update in db: %w", err)
	}

	return nil
}

// Delete soft-deletes an account from the database.
func (s *Store) Delete(ctx context.Context, acct account.Account) error {
	err := pgsql.NamedExecContext(ctx, s.log, s.db, accountDeleteSQL, toAccountDB(acct))
	if err != nil {
...
func (s *Store) QueryByID(ctx context.Context, accountID uuid.UUID) (account.Account, error) {
	...
	var dbAcct accountDB

	err := pgsql.NamedQueryStruct(ctx, s.log, s.db, accountQueryByIDSQL, data, &dbAcct)
	if err != nil {
...
func (s *Store) Query(
	...
	var dbAccounts []accountDB

	err = pgsql.NamedQuerySlice(ctx, s.log, s.db, buf.String(), data, &dbAccounts)
	if err != nil {
		return nil, fmt.Errorf("error query account in db: %w", err)
	}

	return toAccountsDomain(dbAccounts)
}
