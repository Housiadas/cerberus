...
func applyFilter(filter invoice_item.QueryFilter, data map[string]any, buf *bytes.Buffer) {
	wc := make([]string, 0)
	...
	if filter.InvoiceID != nil {
		data["invoice_id"] = *filter.InvoiceID

		wc = append(wc, "invoice_id = :invoice_id")
	}
