...
func TestService_Create_Successful(t *testing.T) {
	...
	expectedEntry := email_notification_outbox2.New(id, no.EventType, no.ToEmail, payloadBytes, 0, now, nil)
	...
	mStorer := email_notification_outbox2.NewMockStorer(t)
...
func TestService_QueryUnprocessed_Successful(t *testing.T) {
	...
	entries := []email_notification_outbox2.EmailNotificationOutbox{
		email_notification_outbox2.New(uuid.New(), "type1", "a@example.com", []byte(`{}`), 0, now, nil),
		email_notification_outbox2.New(uuid.New(), "type2", "b@example.com", []byte(`{}`), 1, now, nil),
	}
