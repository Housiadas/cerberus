package email_notification_outbox_test
...
func TestService_Create_Successful(t *testing.T) {
	...
	expectedEntry := New(id, no.EventType, no.ToEmail, payloadBytes, 0, now, nil)
	...
	sut := email_notification_outbox.NewService(mLogger, mStorer, mUuidGen, mClock)
...
func TestService_QueryUnprocessed_Successful(t *testing.T) {
	...
	entries := []EmailNotificationOutbox{
		New(uuid.New(), "type1", "a@example.com", []byte(`{}`), 0, now, nil),
		New(uuid.New(), "type2", "b@example.com", []byte(`{}`), 1, now, nil),
	}
