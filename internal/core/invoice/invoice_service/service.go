...
type Service struct {
	log    logger.Logger
	storer invoice2.Storer
}

// New constructs the service.
func New(log logger.Logger, storer invoice2.Storer) *Service {
	return &Service{
		log:    log,
		storer: storer,
	}
...
func (s *Service) Create(ctx context.Context, inv invoice2.Invoice) error {
...
func (s *Service) Update(ctx context.Context, inv invoice2.Invoice) error {
...
func (s *Service) QueryByStripeID(
	...
	if err != nil {
		return invoice2.Invoice{}, fmt.Errorf("query by stripe id: %w", err)
	}
