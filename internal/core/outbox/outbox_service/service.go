...
func (s *Service) Create(ctx context.Context, no outbox2.NewOutbox) error {
	...
	now := s.clock.Now()
	o := outbox2.New(id, no.EventType, no.AggregateID, no.Topic, payload, 0, now, nil)
