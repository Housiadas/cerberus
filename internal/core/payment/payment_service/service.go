...
type Service struct {
	log    logger.Logger
	storer payment2.Storer
}

// New constructs the service.
func New(log logger.Logger, storer payment2.Storer) *Service {
	return &Service{
		log:    log,
		storer: storer,
	}
...
func (s *Service) Create(ctx context.Context, pmt payment2.Payment) error {
...
func (s *Service) Update(ctx context.Context, pmt payment2.Payment) error {
...
func (s *Service) QueryByStripeID(
	...
	if err != nil {
		return payment2.Payment{}, fmt.Errorf("query by stripe id: %w", err)
	}
