...
// New constructor.
func New(log logger.Logger, storer permission2.Storer, uuidGen uuidgen.Generator) *Service {
...
func (s *Service) Create(
	...
	if err != nil {
		return permission2.Permission{}, fmt.Errorf("permission uuid generate: %w", err)
	}

	now := time.Now()
	p := permission2.New(id, np.Name, now, now, nil)

	err = s.storer.Create(ctx, p)
	if err != nil {
		return permission2.Permission{}, fmt.Errorf("permission create: %w", err)
	}
...
func (s *Service) Update(
	...
	if err != nil {
		return permission2.Permission{}, fmt.Errorf("permission update: %w", err)
	}
...
func (s *Service) Delete(ctx context.Context, p permission2.Permission) error {
...
func (s *Service) QueryByID(ctx context.Context, id uuid.UUID) (permission2.Permission, error) {
	p, err := s.storer.QueryByID(ctx, id)
	if err != nil {
		return permission2.Permission{}, fmt.Errorf("query: permissionID[%s]: %w", id, err)
	}
