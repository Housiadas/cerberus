...
func (s *Service) Create(ctx context.Context, userID uuid.UUID) (reset_token.ResetToken, error) {
	id, err := s.uuidGen.Generate()
	if err != nil {
		return reset_token.ResetToken{}, fmt.Errorf("uuid: %w", err)
	}
	...
	if err != nil {
		return reset_token.ResetToken{}, fmt.Errorf("create: %w", err)
	}
...
func (s *Service) QueryByToken(ctx context.Context, token string) (reset_token.ResetToken, error) {
	tkn, err := s.storer.QueryByToken(ctx, token)
	if err != nil {
		return reset_token.ResetToken{}, fmt.Errorf("query by token: %w", err)
	}
...
func (s *Service) Delete(ctx context.Context, tkn reset_token.ResetToken) error {
