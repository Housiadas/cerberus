...
func TestService_Query_Successful(t *testing.T) {
	...
	sut := user_roles_permissions.NewService(mLogger, mStorer)
...
func TestService_QueryPermissionsByUserID_Empty(t *testing.T) {
	...
	mStorer := NewMockStorer(t)
	mStorer.EXPECT().QueryPermissionsByUserID(ctx, userID).Return([]Permission{}, nil)
