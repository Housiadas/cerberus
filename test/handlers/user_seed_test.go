...
func insertUserSeedData(test *apitest2.Test) (apitest2.SeedData, error) {
	...
	for _, name := range []string{"user:read:all", "user:delete", "user:read", "user:write"} {
		id, err := apitest2.SeedPermission(ctx, test.DB, name)
		if err != nil {
			return apitest2.SeedData{}, fmt.Errorf("seeding permission %s : %w", name, err)
		}
		permIDs[name] = id
	}
	...
	for _, pid := range permIDs {
		if err = apitest2.SeedRolePermission(ctx, test.DB, adminRoleID, pid); err != nil {
	...
	for _, name := range []string{"user:read", "user:write"} {
		if err = apitest2.SeedRolePermission(ctx, test.DB, userRoleID, permIDs[name]); err != nil {
			return apitest2.SeedData{}, fmt.Errorf("seeding user role permission : %w", err)
		}
	}
